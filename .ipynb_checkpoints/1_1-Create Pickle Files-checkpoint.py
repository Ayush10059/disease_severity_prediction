...
core_mimiciv_imgcxr_path = 'data/physionet.org/files/mimic-cxr-jpg/2.1.0/'

# Load your 1% sample IDs
subset_ids = pd.read_csv('data/subset/subset_subject_ids.csv')['subject_id'].unique()
...
# Filter all clinical DataFrames to the 10% sample
for df_name in ['df_admissions', 'df_transfers', 'df_icustays',
                'df_prescriptions', 'df_labevents',
                # add others if needed
               ]:
    if df_name in globals():
        g = globals()[df_name]
        globals()[df_name] = g[g['subject_id'].isin(subset_ids)]
